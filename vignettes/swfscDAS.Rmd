...
One other note is that `das_read` contains a 'tz' argument that specifies the time zone of the date/times in the DAS file. This argument can generally be ignored if data was collected in the local time zone, as the time zone is not used in any other swfscDAS functions. However, if data was collected in a time zone that causes effort to span multiple days, then 'OnEffort' values will be incorrect because `das_process` resets the event sequence to off effort at the beginning of a new day. To fix this, convert the date/times to the local time zone after running `das_read` and before running `das_process`. For instance, if data was collected in UTC but local time is GMT-10, then you could do the following:

```{r readproc tz}
y.read.dt <- das_read(y, skip = 0, tz = "UTC") %>% 
  mutate(DateTime = lubridate::with_tz(DateTime, "Etc/GMT-10"))
```
