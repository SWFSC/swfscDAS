...
#'
#' @details
#' This is the top-level function for chopping processed DAS data into modeling
#' segments (henceforth 'segments'), and assigning sightings and related
#' information (e.g., weather conditions) to each segment. This function returns
#' data frames with all relevant information for the effort segments and
#' associated sightings ('segdata' and 'sightinfo', respectively). Before
#' chopping, the DAS data is filtered for events (rows) where either the
#' 'OnEffort' column is \code{TRUE} or the 'Event' column "E". In other words,
#' the data is filtered for continuous effort sections (henceforth 'effort
#' sections'), where effort sections run from "R" to "E" events (inclusive), and
#' then passed to the chopping function specified using \code{method}. Note that
#' while B events immediately preceding an R are on effort, they are ignored
#' during effort chopping. In addition, all on effort events (other than ? and
#' numeric events) with \code{NA} DateTime, Lat, or Lon values are verbosely
#' removed.
#'
#' If \code{strata.files} is not \code{NULL}, then the effort lines will be
#' split by the user-provided stratum (strata). In this case, a column 'stratum'
#' will be added to the end of the segdata data frame with the user-provided
#' name of the stratum that the segment was in, or \code{NA} if the segment was
#' not in any of the strata. If no name was provided for the stratum in
#' \code{strata.files}, then the value will be "Stratum#", where "#" is the
#' index of the applicable stratum in \code{strata.files}. While the user can
#' provide as many strata as they want, these strata can share boundaries but
#' they cannot overlap. See \code{\link{das_effort_strata}} for more details.
#'
...
#' sections, i.e. each continuous effort section is a single effort segment. See
#' \code{\link{das_chop_section}} for more details about this method.
#'
...
#' observer (see \code{\link{das_sight}}), and in a Beaufort sea state less than
#' or equal to five.
#'
#' @return
